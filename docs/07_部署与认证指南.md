...
本指南说明 UniversalContract 如何从本地参考实现扩展为可横向扩展的 A2A 服务和带用户租户的数字员工平台。核心业务契约不因部署形态而改变：`WorkerTask`、`WorkerResult`、`Harness` 和各框架 adapter 仍保持中立；变化集中在 HTTP 边界、认证授权、会话存储与执行生命周期。
...
数字员工平台额外提供 `AUTH_PROFILE=local|token`：`local` 是显式无认证演示；`token` 使用
用户名/密码、Argon2 哈希、JWT 与 membership 校验。二者均不改变 `common/`、Harness 或编排层。
...
### 数字员工平台的用户、membership 与 JWT

平台的身份事实由 `Tenant`、`UserAccount`、`Membership` 与 `UserDirectory` 契约表达。
注册请求创建一个租户、用户和 `owner` membership；密码仅保存 Argon2 哈希。登录时
`CredentialVerifier.verify()` 校验密码，`TokenService.issue()` 签发 access/refresh token。

后续请求先验证 Bearer token，再以 token subject 查询用户状态，并用 `X-Tenant`（缺省为 token
中的 tenant）调用 `resolve_membership()`。无凭据、无效/过期/已吊销 token 返回 `401`；用户不属于
目标租户返回 `403`。动态 Worker、Skill、MCP Server、编排记录与会话均按 tenant 查询。

HS256 `JwtTokenService` 是平台参考实现。`JWT_SECRET` 是后端签名私钥，不代表用户或租户，也不
验证“前端是否合法”；它只用于保证 token 确由该服务签发且未被篡改。生产环境必须通过 Secret
Manager、Vault、KMS 或部署系统向所有实例注入同一稳定密钥，至少 32 字节，绝不写入镜像或 Git。
轮换密钥会使旧 token 失效，应纳入发布计划。详见[用户认证与隔离](08_用户认证与隔离.md)。

## 5. 本地与生产接入

[demo/react-orchestration-2-agent-fastapi](../demo/react-orchestration-2-agent-fastapi) 仍使用 `ASGITransport` 和显式无认证 profile，因此无需监听端口，适合学习协议调用链。新增的 [demo/rest-a2a-worker](../demo/rest-a2a-worker) 则用 `StaticBearerAuthenticator` 与 `ScopeAuthorizer` 演示受保护路由：无 token 为 `401`，带 `demo-token` 的调用为 `200`，并展示 `tenant_id:session_id` 存储键。两者都只用于本地学习，生产环境不得使用示例 token。

生产服务应在启动时构建与验证 Harness，在 shutdown 时调用 `Harness.close()`，并使用 `/healthz` 和 `/readyz` 作为实例探针；`create_a2a_app()` 已提供两项探针和路由注册。
