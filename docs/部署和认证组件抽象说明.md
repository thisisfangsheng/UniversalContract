...
数字员工平台在同一原则下增加了“用户身份事实”和“token 生命周期”两个可替换边界：
`common/identity.py` 定义账户、租户、membership 与目录协议；`common/security.py` 定义
`TokenService` 与 `TokenRevocation`。SQLAlchemy/Argon2/JWT 只在平台参考实现中出现。
...
| `auth_mode` | `NONE`、`BEARER`、`GATEWAY`、`OIDC` 或 `MTLS` | 记录认证终止方式，便于审计 |
...
| 契约 | 语义 | 平台参考实现 |
|---|---|---|
| `Tenant` | 隔离的基本单位 | `TenantRecord` |
| `UserAccount` | 无原始密码的用户视图 | `UserRecord`（只持久化 `password_hash`） |
| `Membership` | 用户属于哪个租户及角色 | `MembershipRecord`，`(user_id, tenant_id)` 唯一 |
| `UserDirectory` | 查询用户与成员关系的唯一身份事实来源 | `SqlAlchemyUserDirectory` / `InMemoryUserDirectory` |
| `CredentialVerifier` | 输入用户名和凭据，返回用户或拒绝 | `SqlAlchemyUserDirectory` 的 Argon2 校验 |
| `TokenService` | 签发、验证 token | `JwtTokenService`（HS256 参考实现） |
| `TokenRevocation` | logout 后使 token 不再可用 | `InMemoryTokenRevocation` |

`JWT_SECRET` 是 `JwtTokenService` 的服务端签名密钥，既不代表用户/租户，也不会发给前端。
平台先验签 token，再查询用户状态和 membership；仅验签通过不等于可以访问任意 `X-Tenant`。
登录和刷新响应必须返回当前 `tenant_id`，前端据此附带 `X-Tenant`；重新登录后的资源查询仍会落在
同一 tenant 命名空间。当前实现已隔离租户间资源；同 tenant 内按创建者的资源 ACL 留待
`ResourceAuthorizer` 阶段。
