...
认证与租户能力仍按“契约与实现分离”设计：`common/identity.py` 定义用户、租户、成员关系及
可替换的身份目录契约；`common/security.py` 定义认证、授权、token 签发/验证/吊销契约；
`deployment/` 与数字员工平台负责具体 HTTP、JWT、数据库实现。认证后的 tenant 用于会话和
平台资源隔离，原始 token、密码、证书与 IAM claims 不会进入任务、会话或框架适配器。部署细节见
[部署与认证指南](07_部署与认证指南.md)，平台使用说明见[用户认证与隔离](08_用户认证与隔离.md)。
...
flowchart TB
    ...
    subgraph 身份契约["共享中立层 common/identity.py + common/security.py"]
        ID["Tenant / UserAccount / Membership<br/>UserDirectory / CredentialVerifier"]
        SEC["PrincipalContext / TokenService<br/>TokenRevocation / Authenticator"]
    end
...
| `common/identity.py` | 用户、租户、membership 及可替换身份目录/凭据校验契约 |
| `common/security.py` | 认证、授权、token 与 tenant 作用域会话键契约 |
